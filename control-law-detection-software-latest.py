...
def generate_c_code(graph):
    ...
    for node in topo_order:
        ...
        input_vars = [temp_vars.get(i, i) for i in inputs]
        ...
        if node.startswith("Y"):
            ...
            if node in pointer_return_functions:
                variable_declarations.append(f"    double* {temp_var};  // Pointer return from {node}")
...
def check_c_syntax(c_code: str) -> bool:
    with tempfile.NamedTemporaryFile(delete=False, suffix=".c", mode='w') as tmp_file:
        tmp_file.write(c_code)
        tmp_file_path = tmp_file.name

    try:
        # Using gcc (GNU Compiler Collection) to check for syntax errors
        result = subprocess.run(
            ["gcc", "-fsyntax-only", tmp_file_path],  #"-fsyntax-only" means no output file is created
            capture_output=True,
            text=True
        )
...
if __name__ == "__main__":
    try:
        ...
        if not os.path.exists(image_path):
          raise Exception("image not found in path: "+image_path)
        ...
        check_c_syntax(c_code) #checks syntax of generated C code using GCC tool
        save_file(c_code)
